...
### Hotfix (planned, blocker → v0.1.1)
- **TASK-0035 (архитектор, SESSION-2026-06-03-04).** Блокирующий баг: создание первого/любого
  списания зависает на шаге выбора кошелька — `process_amount` шлёт «Загружаю…», но клавиатуру
  рендерит `@router.message(NewChargeStates.wallet)` (ждёт следующего сообщения, которого нет).
  У новых пользователей вдобавок 0 кошельков. e2e это не ловил (слал callback напрямую). Фикс:
  показывать клавиатуру сразу после суммы; рабочий подпоток «Добавить кошелёк»; дефолтный кошелёк
  «Основная карта» новому пользователю (`get_or_create`); онбординг-подсказка на `/start`; e2e-тест.
  Приоритет — **вне очереди, до M6**. Релиз v0.1.1.
