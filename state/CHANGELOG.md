...
- **Декомпозиция M7 — доработки по отзывам реальных пользователей (архитектор, SESSION-2026-06-03-16).**
  ...
  периода — общий форматтер), **TASK-0040** (период→дата + валидация окна периода; решение владельца:
  дата > сегодня и ≤ сегодня+период), **TASK-0041** (группировка «Мои списания» по категориям + кнопки),
  **TASK-0042** (эмодзи-иконки кошельков; решение владельца: эмодзи-пресет), **TASK-0043** (схема
  `Category.notify_chat_ids` + UI управления), **TASK-0044** (дубль напоминаний в чаты категории +
  уведомление владельца при отсутствии прав), **TASK-0045** (UX редактирования: живая карточка).
  Зависимости: 0039→{0041,0042}, 0040→0045, 0039→0045, 0043→0044. Старт исполнителя — TASK-0039.
