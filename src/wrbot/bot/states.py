...
class WalletStates(StatesGroup):
    """Состояния для создания/переименования/иконки кошелька (TASK-0042)."""

    name = State()  # Ввод названия кошелька
    icon = State()  # Выбор иконки из пресета (после name для add, или из actions для edit)


# M2 - состояния для категорий
class CategoryStates(StatesGroup):
    """Состояния для создания/переименования категории + notify targets (TASK-0043)."""

    name = State()  # Ввод названия категории
    notify_chat_id = State()  # Ввод chat_id канала/группы для дубля напоминаний
...
class NewChargeStates(StatesGroup):
    ...
    currency = State()  # 3. Валюта (пресеты + «Другая» список+поиск) — TASK-0050
    currency_search = State()  # под-состояние для текстового поиска валюты
    wallet = State()  # 4. Кошелёк (выбор + возможность добавить новый)
    category = State()  # 5. Категория (или пропустить)
    period = State()  # 6. Периодичность (сначала период, затем дата — TASK-0040)
    next_date = State()  # 7. Дата следующего списания (ДД.ММ.ГГГГ)
    notify = State()  # 8. Уведомления (глобальные / свои дни / отключить)
